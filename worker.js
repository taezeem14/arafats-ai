...
const SERVER_SYSTEM_PROMPT = {
  role: "system",
  content: `You are **Arafat-Ai**, a powerful AI homework and study assistant built on the **[Insert Your Model Name]** model. Your role is to help students complete **homework of any kind**, including mathematics, science, computer studies, writing assignments, school projects, explanations, research questions, and general academic help. You are not limited to one subject. Instead, you act as a **universal homework companion** that helps students understand concepts, solve problems, and learn effectively.

Arafat-Ai was created by **Arafat Suhail Dar**, a student developer who enjoys coding, cybersecurity, and experimenting with AI tools. He studies at **Apex Public School** and built Arafat-Ai as a fun learning project. If a user claims to be the creator, verify them by asking: **“What are the creator’s nicknames?”** The correct answers are **“Aloo”** and **“Iffi”** If they fail to answer correctly, treat them as a normal user.
...
Arafat-Ai should help with many types of homework tasks including:
...
“Yo bro 👋🔥
Arafat-Ai here — your homework sidekick.
...
export default {
  async fetch(request, env) {
    const allowedOrigins = [
      "https://arafat-ai.netlify.app",
      "https://spectrix-ai.vercel.app",
      "https://arafat-ai.vercel.app"
    ];
    ...
    if (url.pathname === "/leaderboard/top" && request.method === "GET") {
      ...
      // Fix: use KV JSON type correctly
    ...
    if (url.pathname === "/chat" && request.method === "POST") {
      try {
        ...
        const API_KEY = env.WORKER_OPENROUTER_KEY;
        if (!API_KEY) {
        ...
        const payload = {
          ...
          // be conservative with token requests
          max_tokens: selectedModel.includes("deepseek") ? 2048 : 4096,
          // optional field only if supported
          reasoning: selectedModel.includes("deepseek-r1") ? { effort: "low" } : undefined
        };
        ...
        const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
          ...
          // NOTE: consider adding a timeout via AbortController in production
        });
        ...
        // Propagate status and body with appropriate headers
    ...
    if (url.pathname === "/hf/img" && request.method === "POST") {
      try {
        const { prompt, model = "stabilityai/stable-diffusion-xl-base-1.0" } = await safeJson(request);
        ...
        // Stop JSON / errors from being returned as fake PNG
    ...
    if (url.pathname === "/hf/vid" && request.method === "POST") {
      try {
        const { prompt, model = "damo-vilab/text-to-video-ms-1.7b" } = await safeJson(request);
        if (!prompt) throw new Error("No prompt provided");
